...
function esEditable(fechaPartido: string) {
  const hoyColombia = new Date(
    new Date().toLocaleString('en-US', { timeZone: 'America/Bogota' })
  )

  const partidoColombia = new Date(
    new Date(fechaPartido).toLocaleString('en-US', {
      timeZone: 'America/Bogota',
    })
  )

  const hoy = new Date(
    hoyColombia.getFullYear(),
    hoyColombia.getMonth(),
    hoyColombia.getDate()
  )

  const diaPartido = new Date(
    partidoColombia.getFullYear(),
    partidoColombia.getMonth(),
    partidoColombia.getDate()
  )
...
export default function PronosticosPage() {
  ...
  async function cargarPronosticos() {
    ...
    const filtrados = (data || [])
      .filter((p: any) => p.partido?.grupo === grupoSeleccionado)
      .filter((p: any) => esEditable(p.partido.fecha_hora))
      .sort(
        (a: any, b: any) =>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-700 mt-2 font-medium">
                Selecciona un grupo y actualiza tus marcadores antes del día del partido.
              </p>
            </div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-12 gap-2">
            {grupos.map((grupo) => (
              <button
                ...
                className={`rounded-2xl px-4 py-3 font-black border-2 transition ${
                  grupoSeleccionado === grupo
                    ? 'bg-green-800 text-white border-green-900 shadow-lg scale-105'
        ...
        <section className="bg-white rounded-3xl shadow-xl p-5 mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-gray-700">
                Solo aparecen partidos que todavía se pueden modificar.
              </p>
            </div>

            <div className="bg-green-100 text-green-900 font-black rounded-2xl px-4 py-2 text-center">
              {pronosticos.length} partidos disponibles
            </div>
          </div>
          ...
          {!cargando && pronosticos.length === 0 && (
            <div className="bg-yellow-100 border border-yellow-400 rounded-2xl p-4 text-yellow-900 font-bold">
              No hay partidos disponibles para actualizar en este grupo.
            </div>
          )}
            ...
            {pronosticos.map((pronostico: any) => (
              ...
              >
                <p className="text-sm text-gray-700 font-bold mb-4">
                  🗓️ {fechaColombia(pronostico.partido.fecha_hora)}
                </p>
